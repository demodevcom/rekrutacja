...
* Author mod:  Aleksander Lurie                                                *
*******************************************************************************/

define('FPDF_VERSION','1.6');

require_once(dirname(__FILE__).'/font_config.inc');
...
var $NewPageCreated;	 // dla nowoutworzonych stron
...
// Część dotycząca generowania kodów kreskowych
...
function FPDF($orientation='P', $unit='mm', $format='A4')
{
	...
	// Załadowanie czcionek
      $this->AddFont('sans','',SANS.'.php');
      $this->AddFont('sans','I',SANSI.'.php');
      $this->AddFont('sans','B',SANSB.'.php');
      $this->AddFont('serif','',SERIF.'.php');
      $this->AddFont('serif','I',SERIFI.'.php');
      $this->AddFont('serif','B',SERIFB.'.php');

// DODATKI DO KODÓW KRESKOWYCH      
...
function SetUTF8($bool){
	$this->utf8=$bool?true:false;
	$this->cellutf8=false;
}
...
{
	//uft-8 check
	$this->utf8?$msg=iconv('UTF-8', 'ISO-8859-2', $msg):'';
...
{
	//uft-8 check
	$this->utf8?$txt=iconv('UTF-8', 'ISO-8859-2', $txt):'';
	//Output a string
...
{
	//uft-8 check
	// WYKOMENTOWANE NA POTRZEBY RAPORTÓW (bo się sypały)
	if (@$this->utf8&&!$this->cellutf8&&$this->NewPageCreated===false) {
...
function MultiCell($w, $h, $txt, $border=0, $align='J', $fill=false)
{
	...
	// WYKOMENTOWANO BO W RAPOTRACH RZUCAŁO BŁĘDAMI
	@$this->utf8?$txt=iconv('UTF-8', 'ISO-8859-2', $txt):'';
	// DODANO ZAMIAST LINII WYKOMENTOWANEJ WYŻEJ
	//$txt=@iconv('UTF-8', 'ISO-8859-2//IGNORE', $txt);
...
// Metoda odpowiedzialna za generowanie kodów kreskowych
	...
	switch($dest)
	{
		case 'I':
			...
			if(php_sapi_name()!='cli')
			{
				...
				if(headers_sent())
					$this->Error('Some data has already been output, can\'t send PDF file');
				header('Content-Length: '.strlen(htmlspecialchars_decode($this->buffer)));
				header('Content-Disposition: inline; filename="'.$name.'"');
				header('Cache-Control: private, max-age=0, must-revalidate');
				header('Pragma: public');
				ini_set('zlib.output_compression','0');
			}
			echo htmlspecialchars_decode($this->buffer);
			break;
		case 'D':
			//Download file
			if(ob_get_length())
		...
		case 'F':
			...
			fwrite($f,htmlspecialchars_decode($this->buffer),strlen(htmlspecialchars_decode($this->buffer)));
			fclose($f);
			break;
		case 'S':
			//Return as a string
