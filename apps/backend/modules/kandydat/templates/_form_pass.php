...
<form id="kandydat_form" action="<?php echo url_for('kandydat/changePass'.(!$form->getObject()->isNew() ? '?id='.$form->getObject()->getId() : '')) ?>" method="post" <?php $form->isMultipart() and print 'enctype="multipart/form-data" ' ?>>
    ...
    <tbody>
    	...
    	?>
      	<?php foreach ($fieldsList as $key): ?>
  	  	<tr>
  	  		<th><?php echo $form[$key]->renderLabel() ?></th>
  	  		<td>
  	  			<?php 
  	  				$attr = array();
