		...
		case 'turystyka i rekreacja':
			...
			if ($('#kandydat_studia_typ_mgr').attr('checked')) {
				...
				specjalnosci = {
						...
						zarzadzanie: 'Zarządzanie promocją regionalną i lokalną', 
						turystyka: 'Zarządzanie turystyką zrównoważoną'
				};			
			}
			break;
	}
...
function jezyki_dla_kierunku(kierunek) {
	...
	$('#kandydat_jezyk').removeAttr('disabled');
	// jezyk1
	var jezyki1 = null;
	switch(kierunek) {
		case 'informatyka':
			jezyki1 = new Array('angielski - podstawowy', 'angielski - średni', 'angielski - zaawansowany');
			break;
		default:
			jezyki1 = new Array('angielski - podstawowy', 'angielski - średni', 'angielski - zaawansowany',
					'niemiecki - podstawowy', 'niemiecki - średni', 'niemiecki - zaawansowany',
					'francuski - podstawowy', 'francuski - średni', 'francuski - zaawansowany',
					'wloski - podstawowy', 'wloski - średni', 'wloski - zaawansowany');
			break;
		}
		if(jezyki1!=null) {	
	...
	switch(kierunek) {
		case 'informatyka':
			...
			break;
		case 'turystyka i rekreacja':
			jezyki2 = new Array('brak', 'angielski - podstawowy', 'angielski - średni', 'angielski - zaawansowany',
								'niemiecki - podstawowy', 'niemiecki - średni', 'niemiecki - zaawansowany',
								'francuski - podstawowy', 'francuski - średni', 'francuski - zaawansowany',
								'wloski - podstawowy', 'wloski - średni', 'wloski - zaawansowany');
	...
	if ($('#kandydat_studia_typ_mgr').attr('checked')) {
		$('#kandydat_jezyk').empty();
		$('#kandydat_jezyk2').empty();
		$('#kandydat_jezyk').attr('disabled',true);
		$('#kandydat_jezyk2').attr('disabled',true);
	}
